...
const PresentationPage = ({ presentation_id }: { presentation_id: string }) => {
  ...
  const fetchUserSlides = async () => {
    ...
    try {
      ...
      console.log(`PdfMakerPage: Data received. Slides count: ${data?.slides?.length}`);
      ...
      console.error("PdfMakerPage: Error fetching slides:", error);
      setContentLoading(false);
    }
  };
              ...
              <>
                {presentationData &&
                  ...
                  presentationData.slides.map((slide: any, index: number) => (
                    <div key={index} className="w-full slide-rendered-marker" data-speaker-note={slide.speaker_note}>
                      <V1ContentRender
                        slide={slide}
                        isEditMode={true}
