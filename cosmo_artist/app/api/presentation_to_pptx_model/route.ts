...
async function getBrowserAndPage(id: string): Promise<[Browser, Page]> {
  ...
  // Log browser console messages to our terminal
  ...
  console.log("PUPPETEER: Page loaded. Waiting for .slide-rendered-marker...");
  
  try {
    await page.waitForSelector(".slide-rendered-marker", { timeout: 30000 });
    console.log("PUPPETEER: Found .slide-rendered-marker!");
  } catch (e) {
    console.log("PUPPETEER TIMEOUT: .slide-rendered-marker did not appear in 30s");
...
async function getSlidesAndSpeakerNotes(page: Page) {
  ...
  const slides = await slides_wrapper.$$(":scope > .slide-rendered-marker");
