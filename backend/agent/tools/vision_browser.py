    ...
    async def execute(self, url: str, instructions: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            return await asyncio.wait_for(
                self._execute_inner(url, instructions), timeout=45
            )
        except asyncio.TimeoutError:
            return {"success": False, "error": "Vision analysis timed out after 45s"}
        except Exception as e:
            logger.error(f"VisionBrowserTool error: {e}")
            return {"success": False, "error": str(e)}

    async def _execute_inner(self, url: str, instructions: str) -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # networkidle timed out — page may still have content, continue
                pass
