...
class SlidesTool:
    ...
    async def execute(self, action: str = "create", **kwargs) -> Dict[str, Any]:
        try:
            return await self._execute_internal(**kwargs)
        except Exception as e:
            logger.error(f"SlidesTool error: {e}")

            # Try fallback: generate simple PPTX without images
            try:
                logger.info("SlidesTool: trying fallback (no images)")
                return await self._execute_fallback(**kwargs)
            except Exception as e2:
                return {
                    "success": False,
                    "error": f"Slides generation failed: {e}. Fallback also failed: {e2}"
                }
    ...
    async def _execute_fallback(self, session_id: str, title: str, slides: list, filename: str = "presentation.pptx", **kwargs) -> Dict[str, Any]:
        try:
            ...
            slide.placeholders[1].text = "Generated by Archimedes Fallback"
            ...
            for s in slides:
                ...
                if content:
                    tf.text = content
                for bullet in s.get("bullet_points", []):
                    p = tf.add_paragraph()
                    p.text = bullet
                    p.level = 1
            ...
            save = await executor.run_command(session_id, f"echo '{pptx_b64}' | base64 -d > {remote_path}", timeout=60)
            if not save.get("success"):
                return {"success": False, "error": f"Fallback save failed: {save.get('error')}"}

            return {"success": True, "output": f"✓ '{filename}' generated via fallback (no images). Path: {remote_path}", "path": remote_path}
        except Exception as e:
