...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('archimedes.openChat', () => {
        if (chatPanel) {
            chatPanel.reveal();
            return;
        }
        chatPanel = vscode.window.createWebviewPanel('archimedesChat', 'Archimedes AI', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
        chatPanel.webview.html = getChatHTML(ARCHIMEDES_URL());
        chatPanel.onDidDispose(() => { chatPanel = undefined; });
    }));
    // Command: Fix selected code
    context.subscriptions.push(vscode.commands.registerCommand('archimedes.fixSelection', async () => {
        ...
        const task = (`Fix this ${language} code from ${filename}:\n\n` +
            `\`\`\`${language}\n${selectedText}\n\`\`\`\n\n` +
    ...
    context.subscriptions.push(vscode.commands.registerCommand('archimedes.explainSelection', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('archimedes.runTask', async () => {
...
async function sendToArchimedes(task) {
    try {
        const resp = await (0, node_fetch_1.default)(`${ARCHIMEDES_URL()}/api/v1/quick-task`, {
            method: 'POST',
        ...
        vscode.window.showErrorMessage(`Archimedes not reachable at ${ARCHIMEDES_URL()}`);
        return null;
    }
}
async function checkServer(url, bar) {
    try {
        const r = await (0, node_fetch_1.default)(`${url}/health`);
        if (r.ok) {
